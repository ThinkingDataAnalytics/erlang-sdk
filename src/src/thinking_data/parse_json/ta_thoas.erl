%%% ta_thoas is based off of Thoas, which is Apache 2.0 licenced.
%%% repository is: https://github.com/lpil/thoas
...
decode(Json, Options) when is_map(Options) ->
    ...
    ta_thoas_decode:decode(Binary, Options).
...
encode(Input, Options) when is_map(Options) ->
    iolist_to_binary(ta_thoas_encode:encode(Input, Options)).
...
encode_to_iodata(Input, Options) ->
    ta_thoas_encode:encode(Input, Options).
